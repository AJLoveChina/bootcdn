...
const jsonApi = "https://api.bootcdn.cn/libs.min.json";

const getLibDetail = (name) => {
  return axios.get(`https://api.bootcdn.cn/libraries/${name}`)
}

module.exports = {
  downloadJsonFileFromBootcdn() {
    return axios.get(jsonApi)
  },
  ...
  getDesc(name, version) {
    ...
    return this.getDetail(name).then(res => res.data).then(res => {
      ...
      var name = res.name;
      const vvv = version ? version : res.version;

      return res.assets.filter(item => {
        var reg = new RegExp(vvv);
